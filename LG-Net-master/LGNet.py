...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope("loss"):
            # Reshape logits and labels to be one-dimensional
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            ...
            f_encoder_i = self.GSR_module_module(feature, inputs['xyz'][i], inputs['neigh_idx'][i], d_out[i],
                                          'Encoder_layer_' + str(i), is_training)

            f_encoder_i = self.DFE_module(f_encoder_i, inputs['xyz'][i], inputs['neigh_idx'][i],
                                            'DFE_layer_' + str(i), is_training)
    ...
    def GSR_module(self, feature, xyz, neigh_idx, d_out, name, is_training):
        f_pc = helper_tf_util.conv2d(feature, d_out // 2, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_lc, lg_volume_ratio = self.context_learning(xyz, f_pc, neigh_idx, d_out, name + 'LFA', is_training)
    ...
    def context_learning(self, xyz, feature, neigh_idx, d_out, name, is_training):
        d_in = feature.get_shape()[-1].value
        f_xyz = self.relative_pos_encoding(xyz, neigh_idx)
        f_xyz = helper_tf_util.conv2d(f_xyz, d_in, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)


        local_rep, g_dis, lg_volume_ratio = self.local_representation(xyz, neigh_idx)
        f_neighbours = self.gather_neighbour(tf.squeeze(feature, axis=2), neigh_idx)
        f_lc = self.DAA_Module(feature, f_neighbours, f_xyz, d_out // 2,
                                     name + 'dis_att_pooling_1', is_training)
        f_xyz = helper_tf_util.conv2d(f_xyz, d_out // 2, [1, 1], name + 'mlp2', [1, 1], 'VALID', True, is_training)
        f_neighbours = self.gather_neighbour(tf.squeeze(f_lc, axis=2), neigh_idx)
        f_lc = self.DAA_Module(feature, f_neighbours, f_xyz, d_out,
                                     name + 'dis_att_pooling_2', is_training)
        return f_lc, lg_volume_ratio

    def local_representation(self, xyz, neigh_idx):
        neighbor_xyz = self.gather_neighbour(xyz, neigh_idx)
        relative_info, relative_alpha, relative_beta, geometric_dis, local_volume = self.relative_pos_transforming(xyz,
                                                                                                                   neigh_idx,
                                                                                                                   neighbor_xyz)
    ...
    @staticmethod
    def DAA_Module(feature, f_feature, f_xyz, d_out, name, is_training):
        '''
        ######DAA########
        Args:
            feature: 中心点特征
        ...
        f_feature_weight = tf.layers.dense(f_new_feature, 2 * d, activation=tf.nn.leaky_relu, use_bias=True,
                                           name=name + 'fc')
        att_scores = f_xyz_weight + f_feature_weight

        att_scores = tf.nn.softmax(att_scores, axis=1)
        f_lc = f_weight * att_scores
        f_lc = tf.reduce_sum(f_lc, axis=1)
        f_lc = tf.reshape(f_lc, [batch_size, num_points, 1, 2 * d])
    ...
    def DFE_module(self, feature, xyz, neigh_idx, name, is_training):
        ...
        xyz_concat = helper_tf_util.conv2d(xyz_concat, d_out // 2, [1, 1], name + 'mlp_xyz', [1, 1], 'VALID', True,
                                           is_training)
        ...
        feat_concat = helper_tf_util.conv2d(feat_concat, d_out // 2, [1, 1], name + 'mlp_feat', [1, 1], 'VALID', True,
                                            is_training)
        ...
        f_encoding_1 = helper_tf_util.conv2d_simple(f_encoding, d_out // 8, [1, 1], name + 'conv_down_1', [1, 1],
                                                    'VALID')
        f_encoding_1 = tf.nn.relu(f_encoding_1)
        f_encoding_2 = helper_tf_util.conv2d_simple(f_encoding, d_out // 8, [1, 1], name + 'conv_down_2', [1, 1],
                                                    'VALID')
        ...
        final_encoding = helper_tf_util.conv2d(final_encoding, d_out, [1, 1], name + 'conv_up', [1, 1], 'VALID', True,
                                               is_training)
